        ...
        <div className="row justify-content-around">
          {data.books.map((book) => (
            <Card key={book.id} className="m-6 sm-12 col-6">
              ...
              <Card.Body className={bookSelected==book.id?'bg-dark text-white':'bg-white'} onClick={setBookSelected.bind(this,book.id)}>{book.name} <button onClick={()=>handleEditBook(book.id)} className="btn btn-primary">{(isEdit&&bookSelected==book.id)?'Info':'Edit'}</button> <button onClick={()=>handleDeleteBook(book.id)} className="btn btn-danger">X</button></Card.Body>
